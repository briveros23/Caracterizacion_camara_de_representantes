...
def grafico_grafos_simples(g,title, layout):
    # Definir el layout del grafo
    layout = g.layout(layout)  # Layout Kamada-Kawai, puedes cambiarlo por otro
    ...
    # Crear el gráfico de dispersión 2D con Plotly
    node_trace = go.Scatter(
        x=x_nodes, 
        y=y_nodes, 
        mode='markers',
        marker=dict(
            size=10,
            color=degrees,  # Color depende del grado
            colorscale='Viridis',  # Escala de color
            colorbar=dict(title='Grado')
        ),
        text=[f'Nodo {i}<br>Grado: {degrees[i]}<br>Nombre:{z}' for i,z in zip(range(len(g.vs)),g.vs['name'])],
    )
...
def calcular_modularidad_metodos_completa(grafo, edge_weights=None):
    """
    Calcula la modularidad de varios métodos de agrupamiento en un grafo.
    
    Parámetros:
        grafo (igraph.Graph): El grafo sobre el que se calcularán las modularidades.
        edge_weights (str, optional): El atributo del grafo que contiene los pesos de las aristas.
        
    Retorna:
        dict: Un diccionario con los nombres de los métodos y sus respectivas modularidades.
    """
    resultados = {}

    # Método de Louvain
    try:
        louvain_clusters = grafo.community_multilevel()
        resultados['Louvain'] = louvain_clusters.modularity
    except Exception as e:
        resultados['Louvain'] = f"Error: {e}"

    # Método de Infomap
    try:
        infomap_clusters = grafo.community_infomap(edge_weights=edge_weights)
        resultados['Infomap'] = infomap_clusters.modularity
    except Exception as e:
        resultados['Infomap'] = f"Error: {e}"

    # Método de Edge Betweenness
    try:
        edge_betweenness_clusters = grafo.community_edge_betweenness(weights=edge_weights).as_clustering()
        resultados['Edge Betweenness'] = edge_betweenness_clusters.modularity
    except Exception as e:
        resultados['Edge Betweenness'] = f"Error: {e}"

    # Método de Walktrap
    try:
        walktrap_clusters = grafo.community_walktrap(weights=edge_weights).as_clustering()
        resultados['Walktrap'] = walktrap_clusters.modularity
    except Exception as e:
        resultados['Walktrap'] = f"Error: {e}"

    # Método de propagación de etiquetas
    try:
        label_propagation_clusters = grafo.community_label_propagation()
        resultados['Label Propagation'] = label_propagation_clusters.modularity
    except Exception as e:
        resultados['Label Propagation'] = f"Error: {e}"

    # Método de Leiden
    try:
        leiden_clusters = grafo.community_leiden(objective_function="modularity", weights=edge_weights)
        resultados['Leiden'] = leiden_clusters.modularity
    except Exception as e:
        resultados['Leiden'] = f"Error: {e}"

    # Método de Fast Greedy
    try:
        fast_greedy_clusters = grafo.community_fastgreedy(weights=edge_weights).as_clustering()
        resultados['Fast Greedy'] = fast_greedy_clusters.modularity
    except Exception as e:
        resultados['Fast Greedy'] = f"Error: {e}"
